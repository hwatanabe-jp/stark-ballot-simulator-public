...
`develop.tfvars` / `main.tfvars` は値の形を示す placeholder です。実運用では `.env.local` または shell environment から `pnpm terraform:backend`、`pnpm terraform:tfvars:develop`、`pnpm terraform:tfvars:main` で `backend.local.hcl` と `*.local.tfvars` を生成します。
