...
この章は、アプリの検証ページでダウンロードした `bundle.zip` を使って、第三者がローカルで行える配布対象アーカイブ単体の最小監査手順をまとめたものです。`/verify` 画面の最終判定を完全再現するものではなく、Step 5–8 で確認できる不変条件は下表の通りです。

`bundle.zip` 単体では通常そろわない、UI の最終判定に関わる材料は次の通りです。
...
これは PoC として意図した範囲です（[配布対象アーカイブ](../verification/bundle-structure.md) の構成も参照してください）。
...
- [用語集](../appendix/glossary.md) — 「検証」と「監査」の使い分けほか
...
- [ZIP ローカル検証（Ubuntu）](audit-bundle.md)
